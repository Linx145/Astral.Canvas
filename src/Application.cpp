...
namespace AstralCanvas
{
    ...
    bool Application::FinalizeGraphicsBackend()
    {
        switch (GetActiveBackend())
        {
            ...
            case Backend_OpenGL:
            {
                // initialize glad (requires context)
                if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress))
                {
                    THROW_ERR("Failed to initialize glad");
                }

                glfwInit();

                // opengl 4.6, minimum required for api is opengl 4.5
                glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 4);
                glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 6);

                //explicitly use the core-profile without backwards-compatible features we no longer need
                ...
                for (usize i = 0; i < windows.count; i++)
                {
                    ...
                    if (shouldContinue)
                    {
                        ...
                        switch (AstralCanvas::GetActiveBackend())
                        {
                            ...
                            #endif
                            #ifdef ASTRALCANVAS_OPENGL
                            case AstralCanvas::Backend_OpenGL:
                            {
                                glfwSwapBuffers((GLFWwindow*)windows.Get(i)->handle);
                                break;
                            }
                            #endif
                            default:
